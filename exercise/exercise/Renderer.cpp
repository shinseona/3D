...
{
	QueryPerformanceCounter(&_currentFrameCounter);

	double time_distance = _currentFrameCounter.QuadPart - _prevFrameCounter.QuadPart;

	if (time_distance > _perFrame)
	{
		_prevFrameCounter = _currentFrameCounter;
		int count = 0;
		if (count++ > 10)
		{
			count = 0;
